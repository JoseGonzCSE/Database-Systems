...
def Q1(_conn):
    ...
    cur=_conn.cursor()
    sql=("""SELECT * FROM warehouse order by w_warehousekey;""")
    cur.execute(sql)
...
def Q4(_conn):
    ...
    with open(os.path.join('input', '4.in')) as f:
        flat_list = [word for line in f for word in line.split()]
    cur=_conn.cursor()
    sql = f"""SELECT DISTINCT w_name, w_capacity 
                FROM 
                    warehouse,nation, region 
                WHERE 
...
def Q5(_conn):
    ...
    with open(os.path.join('input', '5.in')) as f:
        flat_list = [word for line in f for word in line.split()]
    cur=_conn.cursor()
    sql = f"""SELECT REGION, SUM(Capacity)
                FROM
                    (SELECT w_name AS Warehouse1, r_name AS REGION,w_capacity AS Capacity
                    FROM 
                        warehouse, nation, region
                    WHERE    
                        w_nationkey = n_nationkey
		                AND n_regionkey = r_regionkey),
                    (SELECT w_name AS Warehouse2
                    FROM 
                        warehouse, supplier, nation
                    WHERE 
                        w_suppkey = s_suppkey
                        AND s_nationkey = n_nationkey
    ...
    f = open(path, "a")
    f.write('{:>0} {:>10}'.format("region","capacity")+'\n')
